...
namespace WebApiTraining2.Controllers
{
	...
	public class CustomerController : ControllerBase
	{
		...
		[HttpGet("{id}")]
        public async Task<ActionResult<CustomerData>> Get(Guid id, CancellationToken cancellationToken)
        {
            var request = new CustomerDataListRequest();
            var response = await _mediator.Send(request, cancellationToken);

            var customerData = response.CustomerDatas.FirstOrDefault(data => data.CustomerID == id);
		...
		[HttpPut("{id}")]
		public async Task <ActionResult<UpdateCustomerResponse>> Put(Guid id, [FromBody] UpdateCustomerModel model, [FromServices] IValidator<UpdateCustomerRequest> validator, CancellationToken cancellationToken)
		{
			var request = new UpdateCustomerRequest { CustomerID = id, Name = model.Name, Email = model.Email };
			var validationResult = await validator.ValidateAsync(request);
			if (!validationResult.IsValid)
			{
				validationResult.AddToModelState(ModelState);
